...
import { stories, experiences, cityVision, management, siteConfig } from "@/lib/content";
...
export default function Home() {
  return <main className="home-flow">
    ...
    <section id="worlds" className="portal hoops">
      ...
      <div className="shell portal-inner"><div className="portal-copy"><div className="world-word">HOOPS</div><h2>The court is more than a court.</h2><p>Players become visible. Teams become more structured. Games become archives. Events become properties. Basketball becomes a world people can enter.</p><div className="portal-actions"><a className="btn-line" href={siteConfig.hoopsPlatform} target="_blank" rel="noreferrer">Enter FACKTS Hoops <span className="arrow">↗</span></a><Link className="quiet-link" href="/hoops">Explore the Hoops world →</Link></div></div></div>
    </section>
    ...
    <section className="portal music">
      ...
      <div className="shell portal-inner"><div className="portal-copy"><div className="world-word">MUSIC</div><h2>Every sound has a story around it.</h2><p>Artists are only one layer. Producers, rooms, sessions, crews, projects and the business behind the sound are part of the culture too.</p><div className="portal-wave" aria-hidden="true">{[18,34,25,42,14,38,28,48,20,33,15,44].map((h,i)=><span key={i} style={{"--h":`${h}px`,animationDelay:`${i*.08}s`}} />)}</div><div className="portal-actions"><a className="btn-line" href={siteConfig.musicPlatform} target="_blank" rel="noreferrer">Enter FACKTS Music <span className="arrow">↗</span></a><Link className="quiet-link" href="/music">Explore the Music world →</Link></div></div></div>
    </section>

    <section className="collision">
      <div className="shell collision-inner">
        <div className="collision-copy"><div className="eyebrow">Where the worlds will collide · Coming soon</div><h2 className="section-title">BEATS <em>×</em> BUCKETS.</h2><p className="body-xl">VERSUS is an upcoming signal of the wider FACKTS idea: basketball, artists, hoods, dance, cyphers and crowd energy inside one property. Not two departments. One culture engine.</p><div style={{marginTop:30}}><Link className="btn-line" href="/experiences/versus-beats-buckets">Enter VERSUS <span className="arrow">→</span></Link></div></div>
    ...
    <section className="people-section management-home">
      <div className="shell management-grid">
        ...
        <div className="management-copy">
          ...
          <div className="management-list">{management.map(person => <div className="management-row" key={`${person.name}-${person.role}`}><strong>{person.name}</strong><span>{person.role}</span></div>)}</div>
          <div style={{marginTop:26}}><Link href="/people" className="btn-line">Meet the wider FACKTS people <span className="arrow">→</span></Link></div>
        </div>
      </div>
    </section>

    <section className="stories-home">
      <div className="shell"><div className="eyebrow">Stories / archive</div><h2 className="section-title" style={{marginTop:20}}>WHAT CULTURE<br/>LOOKED LIKE HERE.</h2><div className="story-grid">{stories.slice(0,4).map(story => <Link href={`/stories/${story.slug}`} className="story-card" key={story.slug}><Image src={story.image} alt={story.title} fill sizes="(max-width:760px) 100vw, 50vw" /><div className="story-card-content"><div className="meta">{story.category} · {story.city}</div><h3>{story.title}</h3></div></Link>)}</div><div style={{marginTop:30}}><Link href="/stories" className="btn-line">Open the archive <span className="arrow">→</span></Link></div></div>
    </section>

    <section className="experiences-home">
      <div className="shell"><div className="eyebrow">Experiences</div><h2 className="section-title" style={{marginTop:20}}>WHAT IS COMING NEXT.</h2><div className="experience-list">{experiences.map((item,index) => <Link href={`/experiences/${item.slug}`} className="experience-row" key={item.slug}><div className="date">{String(index+1).padStart(2,"0")} / {item.status}</div><h3>{item.title}</h3><div className="place">{item.city}<br/>{item.type}</div><div className="jump">↗</div></Link>)}</div></div>
    </section>
